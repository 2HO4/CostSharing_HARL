...
def create_details_firms(type_firms: str='heterogeneous', n_firms: int=12, cash_flow_total: int=700000, inventory_total: int=140000):
    """
    ...
    if type_firms.lower() == 'heterogeneous':
        portion_cash_flow = cash_flow_total//7
        portion_inventory = inventory_total//7
        portion_size_firms = n_firms//3
        details_firms = [0]*n_firms
        for i in range(portion_size_firms):
            details_firms[i] = {"name": "SMALL                         ", "costs_fixed": 1000, "cash_flow": portion_cash_flow}
            details_firms[i + portion_size_firms] = {"name": "MEDIUM                        ", "costs_fixed": 2000, "cash_flow": portion_cash_flow}
            details_firms[i + 2*portion_size_firms] = {"name": "BIG                           ", "costs_fixed": 3000, "cash_flow": portion_cash_flow}
...
class CostSharing(ParallelEnv):
    ...
    def action_space(self, agent: int):
        ...
        """
        return Box(low=0, high=self._cap_production, shape=(1, ))

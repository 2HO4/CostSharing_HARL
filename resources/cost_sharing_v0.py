...
class CostSharing(ParallelEnv):
    """
    CostSharing is an environment class that represents a cost-sharing game. It is a PettingZoo paraellel environment.
    ...
    def observation_space(self, agent):
        ...
        return Box(
            ...
            high=np.array(
                ...
                [1]*len(self.possible_agents) +  # status of all firms in the economy
    ...
    def update_observations(self):
        status_firms = [0]*len(self.possible_agents)
        for agent in self._agents_remaining:
            status_firms[agent] = 1
        ...
        if self._time_curr == self._time_end:
            ...
            if self._emissions_curr <= self._emissions_max:  # the total emissions are within the limit so all remaining agents receive a reward
                ...
                for agent in agents_remaining:
                    ...
                    rewards[agent] = 2 if self._firms_curr[agent].get_cash_flow() > self._firms[agent].get_cash_flow() else 1  # the firm earns a bonus if it's made a profit
            else:
                for agent in agents_remaining:
                    rewards[agent] = -1
            infos = {a: {} for a in self.agents}
...
if __name__ == "__main__":
    ...
    details = [
        ...
        {'name': 'big0', 'costs_fixed': 10000, 'cash_flow': 1010000, 'inventory': 10000}, 

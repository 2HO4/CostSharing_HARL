    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                ...
                env = LAGEnv(env_args)
            elif env_name == "cost_sharing":
                from resources.environment.cost_sharing_env import (
                    CostSharingEnvironment,
                )

                env = CostSharingEnvironment(env_args)
            else:
...
def make_render_env(env_name, seed, env_args):
    ...
    if env_name == "smac":
        ...
        from resources.environment.cost_sharing_env import CostSharingEnvironment
        
        env = CostSharingEnvironment({**env_args, "render_mode": "human"})
    else:
        print("Can not support the " + env_name + "environment.")
        raise NotImplementedError
